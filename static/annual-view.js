(function () {
  ...
  function markup() {
    return `<div class="annual-intro"><div><span class="vision-tag">Depuis le début de TESIGN</span><h2>TESIGN, en un coup d’œil.</h2><p id="annualCoverage">Chargement de l’historique…</p></div></div>
      ...
      <p class="annual-axes">Barres : euros, à gauche · Courbe : commandes, à droite · * année partielle · Hachures : prévisionnel</p>
  ...
  function readout(index) {
    ...
    $('annualReadout').innerHTML=`<strong>${safe(row.year)}${row.isForecast?' · prévisionnel fondé sur l’historique':row.isPartial?' · année partielle':''}</strong><div>${series.map(s=>`<span><i style="background:${s.color}"></i>${s.label} <b>${safe(s.format(row[s.key]))}</b> <small>${row.isForecast?'':safe(percent(row.comparison?.pct?.[s.key]))}</small></span>`).join('')}</div><small>${safe(row.isForecast?'Estimation centrale fondée sur les trois dernières années ; aucune croissance supplémentaire supposée.':row.comparison?.label || 'Comparaison non disponible')}</small>`;
  }
  function draw() {
    ...
    let svg=`<svg viewBox="0 0 ${W} ${H}" role="group" aria-label="CA, marge estimée et commandes par année, puis scénario 2027"><defs>${money.map(s=>`<pattern id="forecast-${s.key}" width="7" height="7" patternUnits="userSpaceOnUse" patternTransform="rotate(35)"><rect width="7" height="7" fill="${s.color}" fill-opacity=".1"/><line y2="7" stroke="${s.color}" stroke-width="2"/></pattern>`).join('')}</defs><text x="${L}" y="19" fill="#58675c" font-size="13">${money.length?'EUROS':''}</text><text x="${W-R}" y="19" text-anchor="end" fill="#666bbb" font-size="13">${selected.has('orders')?'COMMANDES':''}</text>`;
    rows.forEach((r,i)=>{if(r.isForecast)svg+=`<rect x="${L+step*i}" y="${T-20}" width="${step}" height="${plot+45}" fill="#fbf7eb"/><line x1="${L+step*i}" x2="${L+step*i}" y1="${T-20}" y2="${T+plot+30}" stroke="#bda782" stroke-dasharray="4 5"/><text x="${x(i)}" y="${T-7}" text-anchor="middle" fill="#856b41" font-size="11">PRÉVISIONNEL</text>`});
    for(let i=0;i<=4;i++){
    ...
    if(selected.has('orders')){
      ...
      rows.forEach((r,i)=>{if(known(r.orders))svg+=`<circle cx="${x(i)}" cy="${cy(r.orders)}" r="5" fill="#666bbb" stroke="white" stroke-width="2"/><text x="${x(i)+10}" y="${cy(r.orders)+19}" fill="#55599b" font-size="13" font-weight="700">${number(r.orders)}</text>`});
    }
    rows.forEach((r,i)=>{const label=`${r.year}${r.isForecast?' (prévisionnel)':r.isPartial?' (partiel)':''} : ${series.map(s=>`${s.label} ${s.format(r[s.key])}`).join(', ')}`;svg+=`<rect class="annual-hit" data-annual-index="${i}" x="${L+step*i}" y="${T-20}" width="${step}" height="${plot+50}" fill="transparent" tabindex="0" role="button" aria-label="${safe(label)}"><title>${safe(label)}</title></rect>`;});
  ...
  function render(data,trajectory) {
    ...
    const t=model.totals,c=model.coverage;
    ...
    $('annualTotals').innerHTML=[
      ...
      {label:'Marge totale estimée',value:euro(t.contribution_margin),note:`Avant publicité et charges fixes${known(t.margin_rate)?' · '+new Intl.NumberFormat('fr-FR',{maximumFractionDigits:1}).format(t.margin_rate)+' % du CA':''}`,key:'margin'}
    ].map(s=>`<article class="summary-metric annual-total-${s.key}"><span class="label">${s.label}</span><strong>${safe(s.value)}</strong><small>${safe(s.note)}</small></article>`).join('');
    $('annualTable').innerHTML='<caption>Montants annuels et évolution par rapport à l’année précédente</caption><thead><tr><th scope="col">Année</th><th scope="col">CA</th><th scope="col">Commandes</th><th scope="col">Marge estimée</th></tr></thead><tbody>'+plotRows.map(r=>`<tr class="${r.isForecast?'forecast-row':''}"><th scope="row"><b>${safe(r.year)}${r.isForecast?' · Prévisionnel':r.isPartial?'*':''}</b><small>${safe(r.isForecast?'Estimation centrale issue de l’historique':r.comparison?.label || 'Première période disponible')}</small></th><td>${safe(euro(r.revenue))}${r.isForecast?'':growth(r,'revenue')}</td><td>${safe(number(r.orders))}${r.isForecast?'':growth(r,'orders')}</td><td>${safe(euro(r.contribution_margin))}${r.isForecast?'':growth(r,'contribution_margin')}</td></tr>`).join('')+'</tbody>';
    $('annualForecastNote').hidden=!showForecast;
    const range=forecast.range||{};
    $('annualForecastNote').innerHTML=`<strong>2027 · Une estimation fondée sur les années précédentes</strong><p>${forecast.complete?`Point central : <b>${euro(forecast.revenue)} de CA</b>. Repères historiques bas / haut : <b>${euro(range.revenue?.low)} à ${euro(range.revenue?.high)}</b>.`:'Calcul incomplet : '+safe((forecast.missing||[]).join(' '))}</p><p>Moyenne des trois dernières années : les années complètes et le rythme de l’année en cours ramené sur douze mois. Aucune croissance supplémentaire n’est supposée. Les repères bas et haut reflètent la variation passée, pas des ventes garanties.</p><details><summary>Voir le calcul et les limites</summary><div id="annualForecastBasis"></div><p>2023 reste dans le graphique, mais son année de lancement incomplète est exclue de la projection. L’annualisation ne prévoit pas la saisonnalité. La publicité, les nouveautés, les prix et les stocks futurs peuvent changer les ventes ; leurs effets ne sont pas chiffrés ici. La marge projetée reprend les coûts historiques estimés.</p></details><button type="button" id="annualEditForecast">Voir mon objectif séparément dans Cap 2027 ↗</button>`;
